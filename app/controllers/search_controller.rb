class SearchController < UIViewController
  ...
  attr_accessor :delegate, :searchBar, :resultsView, :searchDisplayController
  ...
  def body_height_for(text)
    ...
    labelSize = text.sizeWithFont(cellFont, constrainedToSize: constraint_size, lineBreakMode: UILineBreakModeWordWrap)
    label_size.height
  end
  ...
  def tableView(table_view, heightForRowAtIndexPath: index_path)
    body_height_for(get_cell_text_at_index_path(indexPath)) + 54
  end
